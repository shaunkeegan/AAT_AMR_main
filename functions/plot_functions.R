...
plot_R0_Sen_wl_dd <- function(df2, Wn, fitadj){
  ...
  pA <- df2 %>% filter(W_st %in% Wn) %>% ggplot() + my_theme() +
    geom_point(aes(x = treat_prop, y = R0_sen, colour = as.factor(K), shape = as.factor(W_st)), size = 3) + 
    #geom_line(aes(x = treat_prop, y = R0_sen, colour = as.factor(Vector_no),shape = as.factor(W_st))) +
    xlab("\n") +
    ylab("")+
    guides(col = guide_legend("Carrying \nCapacity (K)"), shape = guide_legend("Wildlife"))
  pA 
  pA + gghighlight(K == K_vec[3], keep_scales = TRUE)
  
  p <- pA
  p1 <- p + gghighlight(K == K_vec[3],  fit.adj == fitadj,   unhighlighted_params = my_par, keep_scales = TRUE)
  p2 <- p + gghighlight(K == K_vec[2],  fit.adj == fitadj,   unhighlighted_params = my_par, keep_scales = TRUE)
  p3 <- p + gghighlight(K == K_vec[1],  fit.adj == fitadj,   unhighlighted_params = my_par, keep_scales = TRUE)
  
  legend <- get_legend(p + theme(legend.box.margin = margin(0, 0, 0, 12), legend.title=element_text(size=15)))
  plot_grid(p3 + theme(legend.position="none") + geom_line(aes(x = treat_prop, y = R0_sen, colour = as.factor(K),shape = as.factor(W_st))) + ylab("R0 sensitive"), 
            p2 + theme(legend.position="none") + geom_line(aes(x = treat_prop, y = R0_sen, colour = as.factor(K),shape = as.factor(W_st))) + xlab("Treatment\n"),  
            p1 + theme(legend.position="none") + geom_line(aes(x = treat_prop, y = R0_sen, colour = as.factor(K),shape = as.factor(W_st))), legend, ncol = 4)
...
plot_res_v_sen_dd <- function(df2, Wn, fitadj){
  ...
  p1 <- df2 %>% filter(W_st %in% Wn) %>% filter(fit.adj %in% fitadj)  %>% ggplot() + my_theme() +
    ...
    xlab("Treatment") + ylab("R resistant / R sensitive") + ylim(0,10)  + ggtitle(paste0("Fitness = ",fitadj)) +
...
plot_prev_v_trt_dd <- function(df2, Wn, fitadj){
  ...
  p <- df2 %>% filter(W_st %in% Wn) %>%  filter(fit.adj %in% fitadj)  %>% ggplot() + my_theme() +
    geom_point(aes(x = treat_prop, y = prevalence, colour = as.factor(W_st), shape = as.factor(K)), size = 3) + 
  ...
  plot_grid(p3 + theme(legend.position="none") + geom_line(aes(x = treat_prop, y = prevalence, colour = as.factor(W_st), shape = as.factor(K))) + ylab("Prevalence"), 
            p2 + theme(legend.position="none")+ geom_line(aes(x = treat_prop, y = prevalence, colour = as.factor(W_st), shape = as.factor(K))) + xlab("Proportion\nTreated"), 
            p1 + theme(legend.position="none")+ geom_line(aes(x = treat_prop, y = prevalence, colour = as.factor(W_st), shape = as.factor(K))), legend, ncol = 4)
...
plot_inc_v_trt_dd <- function(df2, Wn, fitadj){
  ...
  p <- df2 %>% filter(W_st %in% Wn) %>% ggplot() + my_theme() +
    geom_point(aes(x = treat_prop, y = Incidence, colour = as.factor(W_st), shape = as.factor(K)), size = 3) + 
    #geom_line(aes(x = treat_prop, y = Incidence, colour = as.factor(W_st), shape = as.factor(Vector_no))) +
    xlab("\n") +
    guides(col = guide_legend("Wildlife"), shape = guide_legend("Carrying \nCapacity (K)"))
  p
  ...
  legend <- get_legend(p + theme(legend.box.margin = margin(0, 0, 0, 12)))
  plot_grid(p3 + theme(legend.position="none") + geom_line(aes(x = treat_prop, y = Incidence, colour = as.factor(W_st), shape = as.factor(K)))+ ylab("Incidence"), 
            p2 + theme(legend.position="none") + geom_line(aes(x = treat_prop, y = Incidence, colour = as.factor(W_st), shape = as.factor(K)))+ xlab("Proportion\nTreated"), 
            p1 + theme(legend.position="none") + geom_line(aes(x = treat_prop, y = Incidence, colour = as.factor(W_st), shape = as.factor(K))), legend, ncol = 4)
...
plot_trtcat_v_trt_dd <- function(df2, Wn, fitadj){
  ...
  p <- df2 %>% filter(W_st %in% Wn) %>% ggplot() + my_theme() +
    geom_point(aes(x = treat_prop, y = No_trt_cat, colour = as.factor(W_st), shape = as.factor(K)), size = 3) + 
    #geom_line(aes(x = treat_prop, y = Incidence, colour = as.factor(W_st), shape = as.factor(Vector_no))) +
    xlab("\n") + ylab("") +
    guides(col = guide_legend("Wildlife"), shape = guide_legend("Carrying \nCapacity (K)"))
  p
  ...
  legend <- get_legend(p + theme(legend.box.margin = margin(0, 0, 0, 12)))
  plot_grid(p3 + theme(legend.position="none") + geom_line(aes(x = treat_prop, y = No_trt_cat, colour = as.factor(W_st), shape = as.factor(K)))+ ylab("Number of Treated Cattle"), 
            p2 + theme(legend.position="none") + geom_line(aes(x = treat_prop, y = No_trt_cat, colour = as.factor(W_st), shape = as.factor(K)))+ xlab("Proportion\nTreated"), 
            p1 + theme(legend.position="none") + geom_line(aes(x = treat_prop, y = No_trt_cat, colour = as.factor(W_st), shape = as.factor(K))), legend, ncol = 4)
...
plot_onward_v_trt_dd <- function(df2, Wn, fitadj){
  ...
  p <- df2 %>% filter(W_st %in% Wn) %>% ggplot() + my_theme() +
    geom_point(aes(x = treat_prop, y = Prob_onward_tran, colour = as.factor(W_st), shape = as.factor(K)), size = 3) + 
    ylim(0.5,1) +
    #geom_line(aes(x = treat_prop, y = Incidence, colour = as.factor(W_st), shape = as.factor(Vector_no))) +
    xlab("\n") + ylab("") +
    guides(col = guide_legend("Wildlife"), shape = guide_legend("Carrying \nCapacity (K)"))
  ...
  legend <- get_legend(p + theme(legend.box.margin = margin(0, 0, 0, 12)))
  plot_grid(p3 + theme(legend.position="none")+geom_line(aes(x = treat_prop, y = Prob_onward_tran, colour = as.factor(W_st), shape = as.factor(K)))+ ylab("Prbability of onward transmission"), 
            p2 + theme(legend.position="none")+geom_line(aes(x = treat_prop, y = Prob_onward_tran, colour = as.factor(W_st), shape = as.factor(K)))+ xlab("Proportion\nTreated"), 
            p1 + theme(legend.position="none")+geom_line(aes(x = treat_prop, y = Prob_onward_tran, colour = as.factor(W_st), shape = as.factor(K))), legend, ncol = 4)
...
plot_risk_v_trt_dd <- function(df2, Wn, fitadj){
  ...
  #Overall risk
  p <- df2 %>% filter(W_st %in% Wn) %>% ggplot() + my_theme() +
    geom_point(aes(x = treat_prop, y = Risk, colour = as.factor(W_st), shape = as.factor(K)), size = 3) + 
    #geom_line(aes(x = treat_prop, y = Incidence, colour = as.factor(W_st), shape = as.factor(Vector_no))) +
    xlab("\n") + ylab("") +
    guides(col = guide_legend("Wildlife"), shape = guide_legend("Carrying \nCapacity (K)"))
  p
  ...
  legend <- get_legend(p + theme(legend.box.margin = margin(0, 0, 0, 12)))
  plot_grid(p3 + theme(legend.position="none")+geom_line(aes(x = treat_prop, y = Risk, colour = as.factor(W_st), shape = as.factor(K)))+ ylab("Relative risk of AMR emergence"), 
            p2 + theme(legend.position="none")+geom_line(aes(x = treat_prop, y = Risk, colour = as.factor(W_st), shape = as.factor(K)))+ xlab("Proportion\nTreated"), 
            p1 + theme(legend.position="none")+geom_line(aes(x = treat_prop, y = Risk, colour = as.factor(W_st), shape = as.factor(K))), legend, ncol = 4)
