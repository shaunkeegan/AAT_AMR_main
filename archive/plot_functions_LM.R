...
plot_R0_Sen_wl_dd_LM <- function(df2, Wn, fitadj){
  ################################################################################
  my_theme <- function () { 
    theme_grey(base_size=16) +
      theme(plot.title = element_text(hjust = 0.5),
  ...
  pA <- df2 %>% filter(W_st %in% Wn) %>% ggplot() + my_theme() +
    ...
    guides(col = guide_legend("Vectors"), shape = guide_legend("Wildlife"))
  pA 
  pA + gghighlight(eq_pop == eq_pop_vec[3], keep_scales = TRUE)
  
  p <- pA
  p1 <- p + gghighlight(eq_pop == eq_pop_vec[3],  fit.adj == fitadj,   unhighlighted_params = my_par, keep_scales = TRUE)
  p2 <- p + gghighlight(eq_pop == eq_pop_vec[2],  fit.adj == fitadj,   unhighlighted_params = my_par, keep_scales = TRUE)
  p3 <- p + gghighlight(eq_pop == eq_pop_vec[1],  fit.adj == fitadj,   unhighlighted_params = my_par, keep_scales = TRUE)
