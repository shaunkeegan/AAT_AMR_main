...
for (birth.adj in birth.adj.vec) {
  print(paste0("loop1, birth.adj = ", birth.adj, "; Iteration ", which(birth.adj == birth.adj), " of ", length(birth.adj)))
      ...
      for (K in K.vec) {
        ...
        for (NW in N_wl) {
          ...
          for (prop_treat in Trt_pop) {
            print(paste0("loop6, prop_treat = ", prop_treat, "; Iteration ", which(prop_treat == Trt_pop), " of ", length(Trt_pop)))
            ## Parameters ----
            ...
            feed.cyc <- 4
            #p <- 0.1
            death.v <- -1 * log((1 - prop.insecticide) * qf * qn ^ feed.cyc) / feed.cyc
            
            birth.v = birth.adj *(-1) * log((1 - 0) * qf * qn ^ feed.cyc) / feed.cyc
